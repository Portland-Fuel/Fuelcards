using System;
using System.Collections.Generic;

namespace DataAccess.Cdata;

public partial class XeroCustomer
{
    public int PortlandId { get; set; }

    public string? Contactid { get; set; }

    public string? Contactname { get; set; }

    public string? Accountnumber { get; set; }

    public string? Emailaddress { get; set; }

    public string? Firstname { get; set; }

    public string? Lastname { get; set; }

    public string? Poattentionto { get; set; }

    public string? Poaddressline1 { get; set; }

    public string? Poaddressline2 { get; set; }

    public string? Poaddressline3 { get; set; }

    public string? Poaddressline4 { get; set; }

    public string? Pocity { get; set; }

    public string? Poregion { get; set; }

    public string? Popostalcode { get; set; }

    public string? Pocountry { get; set; }

    public string? Saattentionto { get; set; }

    public string? Saaddressline1 { get; set; }

    public string? Saaddressline2 { get; set; }

    public string? Saaddressline3 { get; set; }

    public string? Saaddressline4 { get; set; }

    public string? Sacity { get; set; }

    public string? Saregion { get; set; }

    public string? Sapostalcode { get; set; }

    public string? Sacountry { get; set; }

    public string? Phonenumber { get; set; }

    public string? Faxnumber { get; set; }

    public string? Mobilenumber { get; set; }

    public string? Ddinumber { get; set; }

    public string? Skypename { get; set; }

    public string? Bankaccountname { get; set; }

    public string? Bankaccountnumber { get; set; }

    public string? Bankaccountparticulars { get; set; }

    public string? Taxnumber { get; set; }

    public string? Accountsreceivabletaxcodename { get; set; }

    public string? Accountspayabletaxcodename { get; set; }

    public string? Website { get; set; }

    public string? Legalname { get; set; }

    public string? Discount { get; set; }

    public string? Companynumber { get; set; }

    public short? Duedatebillday { get; set; }

    public string? Duedatebillterm { get; set; }

    public short? Duedatesalesday { get; set; }

    public string? Duedatesalesterm { get; set; }

    public string? Salesaccount { get; set; }

    public string? Purchasesaccount { get; set; }

    public string? Trackingname1 { get; set; }

    public string? Salestrackingoption1 { get; set; }

    public string? Purchasestrackingoption1 { get; set; }

    public string? Trackingname2 { get; set; }

    public string? Salestrackingoption2 { get; set; }

    public string? Purchasestrackingoption2 { get; set; }

    public string? Brandingtheme { get; set; }

    public string? Defaulttaxbills { get; set; }

    public string? Defaulttaxsales { get; set; }

    public string? Person1firstname { get; set; }

    public string? Person1secondname { get; set; }

    public string? Person1email { get; set; }

    public string? Person2firstname { get; set; }

    public string? Person2secondname { get; set; }

    public string? Person2email { get; set; }

    public string? Person3firstname { get; set; }

    public string? Person3secondname { get; set; }

    public string? Person3email { get; set; }

    public string? Person4firstname { get; set; }

    public string? Person4secondname { get; set; }

    public string? Person4email { get; set; }

    public string? Person5firstname { get; set; }

    public string? Person5secondname { get; set; }

    public string? Person5email { get; set; }
}
