...
namespace FuelcardModels.Operations
{
    ...
    public class MemoriseE22
    {
        ...
        public MemoriseE22(string FilePath)
        {
            _filePath = FilePath;
            TestFilePath();
            Import = new E22();
            Import.E22Details = new List<E22Detail>();
        }
        ...
        private void TestFilePath()
        {
            ...
            if (_filePath.Substring(_filePath.Length - 3) != "E22") throw new ArgumentException($"The file {_filePath} is not an E22 file type, please check the file and try again.");
        }
        ...
        private void ParseDetailRecord(string line)
        {
            ...
            d.Time = new TimeOnly8(p[4]);
            d.StopStatusCode = new GenericChar(p[5]);
            d.PersonWhoRequestedStop = new Name(p[6]);
            d.StopReferenceNumber = new Int5(p[7]);

            Import.E22Details.Add(d);
        }
        ...
        {
            if (Import.E22Details.Count != Import.E22Control.RecordCount.Value) return false;
            return true;
        }
    }
}
